...
- Resend email API, configured by the operator and called directly over HTTPS.
...
flowchart TB
  ...
  Worker --> Coach
  Worker --> Providers["Direct OpenAI and Resend HTTPS clients"]
...
| Provider quota or Resend delivery fails during judging | Keep deterministic features usable, label cached results, provide explicit retry, validate provider configuration during setup, and prepare the seeded household before the demo window. |
...
- [Excelize](https://github.com/qax-os/excelize), [ledongthuc/pdf](https://github.com/ledongthuc/pdf), [SQLite FTS5](https://www.sqlite.org/fts5.html), [SQLite VACUUM INTO](https://www.sqlite.org/lang_vacuum.html#vacuuminto), [Resend send email](https://resend.com/docs/api-reference/emails/send-email), and [RFC 5545](https://datatracker.ietf.org/doc/html/rfc5545).
...
- **Test scenarios:**
  ...
  - Install and reconfigure stage configuration, preserve the systemd-delivered master key and data, validate Resend/allowlist inputs, rotate the Resend credential atomically, and roll back every owned file on failure.
  ...
  - Default uninstall removes the installer binary, service/timer, non-secret runtime config, Resend credential, and owned proxy artifacts while preserving recovery state; confirmed purge removes only the enumerated Mithra data, backups, journal, and master key.
...
- **Test scenarios:**
  ...
  - Resend bootstrap and both judge accounts work in fresh sessions; unknown email remains indistinguishable and blocked, while no password, reset URL, API key, or session material enters the repo, README, video, fixtures, logs, or deployment receipt.
...
| Provider contract | Local fake OpenAI and Resend servers plus one explicit live smoke | Requests use the expected endpoints, schemas, `store: false`, data-minimization, idempotency, timeout, and error handling. | U2-U5, U9, U11 |
