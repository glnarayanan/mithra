...
func TestOwnerOpenAISettingsValidateEncryptPreserveAndRemove(t *testing.T) {
	...
	save := authenticatedSettingsRequest(owner, http.MethodPost, url.Values{"action": {"save_openai"}, "api_key": {firstKey}})
	saved := serve(application, save)
	if saved.Code != http.StatusOK || !strings.Contains(saved.Body.String(), "OpenAI is connected") || strings.Contains(saved.Body.String(), firstKey) {
	...
	if removed.Code != http.StatusOK || !strings.Contains(removed.Body.String(), "OpenAI was disconnected") || !strings.Contains(removed.Body.String(), "Not connected") {
